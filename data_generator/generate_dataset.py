    ...
    def build_identifiers(self):
        for p in self.persons:
            ...
            for k in range(n_ph):
                ...
                # The primary number predates the observation window and is
                # never surrendered, so every subject always has at least one
                # SIM live at any timestamp. Without that guarantee the CDR
                # would have to invent calls on SIMs that were not yet issued.
                act = (T_START - timedelta(days=self.rng.randrange(30, 900))
                       if k == 0 else self.rdate())
                deact = ""
    ...
    def build_cdr(self, path):
        ...
        *lossy* projection of the truth, which is exactly the analytic problem.
        ...
        with open(path, "w", newline="", encoding="utf-8") as fh:
            ...
            # A SIM can only carry a call while it is actually in service.
            # Picking uniformly from a person's SIMs produces traffic on numbers
            # that were not yet issued or had already been surrendered -- which
            # both contradicts the subscriber registry and destroys the SIM-
            # rotation signal the dataset is supposed to contain.
            life = {}
            for ph in self.phones:
                a = datetime.fromisoformat(ph["activation_date"])
                d = (datetime.fromisoformat(ph["deactivation_date"])
                     if ph["deactivation_date"] else T_END + timedelta(days=1))
                life[ph["phone_id"]] = (a, d)

            def active_phone(pid, ts):
                cands = [ph for ph in self.phones_of[pid]
                         if life[ph["phone_id"]][0] <= ts <= life[ph["phone_id"]][1]]
                return self.rng.choice(cands) if cands else None

            def emit(pa, pb, ts, ctype=None):
                pha = active_phone(pa, ts)
                phb = active_phone(pb, ts)
                if pha is None or phb is None:
                    return
                emit_on(pha, phb, pa, pb, ts, ctype)

            def emit_on(pha, phb, pa, pb, ts, ctype=None):
            ...
            for p in rotators:
                ...
                if not peers:
                    continue
                # The rotation must follow the SIM's real service life: the
                # discarded number must go silent when it is surrendered, and
                # the replacement must pick the same contacts up afterwards.
                # A cut date unrelated to the registry would leave both SIMs
                # active throughout and encode no rotation at all.
                dropped = next((ph for ph in phs
                                if ph["is_burner"] and ph["deactivation_date"]), None)
                if dropped is None:
                    continue
                cut = datetime.fromisoformat(dropped["deactivation_date"])
                start = datetime.fromisoformat(dropped["activation_date"])
                successor = next((ph for ph in phs
                                  if ph["phone_id"] != dropped["phone_id"]), None)
                if successor is None or cut <= start:
                    continue
                window = max(1, (cut - start).days)
                contacts = self.rng.sample(peers, min(len(peers), self.rng.randint(2, 5)))
                for _ in range(self.rng.randint(10, 30)):
                    q = self.rng.choice(contacts)
                    ts = cut - timedelta(days=self.rng.uniform(1, window))
                    qp = active_phone(q, ts)
                    if qp:
                        emit_on(dropped, qp, p["person_id"], q, ts)
                for _ in range(self.rng.randint(10, 30)):
                    q = self.rng.choice(contacts)
                    ts = cut + timedelta(days=self.rng.uniform(1, 120))
                    qp = active_phone(q, ts)
                    if qp and ts <= T_END:
                        emit_on(successor, qp, p["person_id"], q, ts)
                self.gt_anomalies.append({
                    "anomaly_id": self.nid("ANOM"),
